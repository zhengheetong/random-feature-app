...
export default function Objectives() {
  ...
  return (
    <div style={{ padding: isObs ? '10px' : '20px', display: 'flex', flexDirection: 'column', alignItems: isObs ? 'flex-start' : 'center' }}>
      ...
      <div style={{
        ...
        <h2 style={{ margin: '0 0 15px 0', fontSize: '20px', color: '#f1c40f', textTransform: 'uppercase', letterSpacing: '1px' }}>
          Current Objectives
        </h2>
        
        {tasks.length === 0 ? (
          <div style={{ color: '#7f8c8d', fontStyle: 'italic', fontSize: '15px' }}>No active objectives.</div>
        ) : (
      ...
      {!isObs && (
        <div style={{ 
          marginTop: '30px', padding: '20px', backgroundColor: '#ffffff', borderRadius: '12px',
          boxShadow: '0 4px 6px rgba(0,0,0,0.05)', minWidth: '350px', textAlign: 'left'
        }}>
          <h3 style={{ marginTop: 0, color: '#2c3e50', marginBottom: '15px', textAlign: 'center' }}>Objective Controls</h3>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
            <input 
              ...
              style={{ flex: 1, padding: '10px', fontSize: '15px', borderRadius: '6px', border: '1px solid #bdc3c7', outline: 'none' }}
              onKeyDown={(e) => e.key === 'Enter' && addTask()}
            />
            <button 
              onClick={addTask}
              style={{ padding: '10px 15px', backgroundColor: '#3498db', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
            >
              Add
            </button>
          </div>

          <hr style={{ border: 'none', borderTop: '1px solid #ecf0f1', marginBottom: '15px' }} />

          {/* Manage Tasks List */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px', maxHeight: '200px', overflowY: 'auto' }}>
            {tasks.map(task => (
              <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#f8f9fa', padding: '8px 10px', borderRadius: '6px', border: '1px solid #e1e8ed' }}>
                <input 
                  type="checkbox" 
                ...
                />
                <span style={{ flex: 1, fontSize: '15px', color: task.completed ? '#95a5a6' : '#2c3e50', textDecoration: task.completed ? 'line-through' : 'none' }}>
                  {task.text}
                </span>
                <button 
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
             <button 
              onClick={clearAll}
              style={{ flex: 1, padding: '10px', backgroundColor: '#bdc3c7', color: '#2c3e50', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
            >
              Clear All
            </button>
            <button 
              onClick={handleCopyObsLink}
              style={{ flex: 2, padding: '10px', backgroundColor: copied ? '#27ae60' : '#2c3e50', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', transition: 'background-color 0.2s' }}
            >
